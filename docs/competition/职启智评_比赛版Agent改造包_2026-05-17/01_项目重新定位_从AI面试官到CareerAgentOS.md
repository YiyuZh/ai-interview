...
| 用户价值 | 模拟面试 | 诊断能力缺口、追问证据、生成学习路径、持续训练 |
...
```text
职启智评通过 Career-AgentOS 多智能体架构，把岗位画像、简历证据、能力缺口、模拟面试、报告生成、学习任务、人工评分和模型后训练串成一条闭环，让每一次求职训练都能反哺下一轮诊断能力优化。
```
...
flowchart TD
    ...
    OPC --> A6[学习任务 Agent]
    OPC --> A7[数据治理 Agent]
    OPC --> A8[Eval Judge Agent]
    OPC --> A9[SFT/DPO/QLoRA 路线 Agent]
...
每个 Agent 只负责一个明确任务：岗位画像约束、简历证据识别、能力缺口定位、面试追问、报告生成、数据治理、评估和后训练。
